...
## as of 2025 analyses, this only summarises the convergence information
## the get_summary() function in bbsBayes2 is now run during model fitting
...
## this is now done in the GPSC during model fitting
...
if(nrow(sp_not_run_but_should) > 0){
  ...
  for(i in 1:nrow(sp_not_run_but_should)){
    aou <- as.integer(sp_not_run_but_should[i,"aou"])
    if(file.exists(paste0(output_dir,"/fit_",aou,".rds"))){
      sp_not_run_but_should[i,"re_converge"] <- TRUE
    }else{
      sp_not_run_but_should[i,"re_converge"] <- FALSE
    }
  }

  warning(paste(nrow(sp_not_run_but_should),"species or more are missing, including",
                paste(sp_not_run_but_should$english,collapse = ", "),
                "confirm that they each have < 1 stratum or that their convergence diagnostics are missing"))
...
n_cores <- nrow(sp_re_run_converge)

cluster <- makeCluster(n_cores, type = "PSOCK")
registerDoParallel(cluster)


test <- foreach(i = rev(1:nrow(sp_re_run_converge)),
                .packages = c("bbsBayes2",
                              "tidyverse",
                              "cmdstanr"),
                .errorhandling = "pass") %dopar%
  {
    ...
    aou <- as.integer(sp_re_run_converge[i,"aou"])
      fit <- readRDS(paste0(output_dir,"/fit_",aou,".rds"))
